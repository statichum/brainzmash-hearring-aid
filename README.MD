...
Artist posters fall back to Deezer only when Fanart.tv and TheAudioDB do not
provide a `Poster` and MusicBrainz has an explicit Deezer artist URL relationship.
There is no artist-name matching and Deezer is not used for banners. Artist
metadata and JPEGs are fetched through the separately cached
`tadb.brainzmash.cc/deezer/` routes; deploy the updated TADB proxy before
updating LMD. If Deezer is unavailable, the artist response still succeeds
without that fallback image.

The BrainzMash community metadata pool supports LMD Metadata requests for both Lidarr and Aurral.
It also provides the /ws/2 MusicBrainz endpoints (read only) for Droppedneedle metadata requests.


This guide will help you deploy a local MusicBrainz mirror with the LMD add-in and NGINX proxy for securely to allowing connection to the Brainzmash server pool.
...
- The BrainzMash server pool is a load balanced community group of Lidarr Metadata servers. We pool our resources to provide this working metadata to Lidarr, Aurral and Droppedneedle.

- Your server stays private, you share the address and generated API key with me privately, I add it to the pool, that's it. You'll only recieve traffic from the Brainzmash frontdoor server, not direct from end users.
...
- The froontdoor server caches responses to significantly reduce load on backend servers like yours. Usage. is high enough that cache is always warm and backend server hits are fairly minimal for the overall traffic volume.
...
- Replication status of each server is checked every 6 hours to make sure it's within 24h, if outside, any lagging server will be rejected until next check.
- Dashboard for server stats is here: https://dash.brainzmash.cc/
...
- The recording search index is large and substantially increases indexing load
  and disk usage; make sure the host has ample free SSD/NVMe space before starting.
...
- (privately) 👋🏼 https://discord.gg/MSTteNvANt

---

## 4. Build and Initialize MusicBrainz Database
Note this step will take hours - perhaps many hours - Run it in tmux or screen so that you do not lose the session.

```bash
docker compose build
docker compose run --rm musicbrainz createdb.sh -fetch   # This may take an hour or more
docker compose up -d
docker compose exec indexer python -m sir reindex --entity-type artist --entity-type release-group --entity-type release --entity-type recording  # Indexing may take several hours
```
...
```
0 1 * * 7 root cd /opt/docker/musicbrainz-docker && /usr/bin/docker compose exec -T indexer python -m sir reindex --entity-type artist --entity-type release-group --entity-type release --entity-type recording
```
